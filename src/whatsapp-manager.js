...
async function getGroupMembers(index, groupId) {
  ...
  try {
    ...
    return (meta.participants || []).map((p) => {
      // Newer WhatsApp uses LID system — participant id may be @lid instead of @s.whatsapp.net
      // Check all possible fields for a real phone JID (@s.whatsapp.net)
      ...
      return {
        id: p.id,            // keep original for API calls (Baileys needs this exact JID)
        number,              // extracted phone number for display & search
...
function startAutoAccept(accountIndex, groupId, intervalMs) {
  ...
  const timer = setInterval(async () => {
    try {
      ...
      // STRICT: Only accept invite_link requests — skip non_admin_add AND unknown
      const linkOnly = list.filter((r) => r.method === "invite_link");
