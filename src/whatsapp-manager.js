...
// method: "invite_link" = joined via group link | "non_admin_add" = added by a member
async function getGroupPendingRequests(index, groupId) {
  ...
  try {
    ...
    return (list || []).map((p) => {
      ...
      // Capture method: invite_link or non_admin_add (or unknown)
      const method = (typeof p === "object" && p !== null)
        ? (p.method || p.requestMethod || "unknown")
        : "unknown";
      return { id: jid, number, method };
    });
  } catch (err) {
    console.error(`[WA${index + 1}] getGroupPendingRequests error:`, err.message);
    return [];
  }
}
...
function startAutoAccept(accountIndex, groupId, intervalMs) {
  ...
  const timer = setInterval(async () => {
    try {
      const pending = await getGroupPendingRequests(accountIndex, groupId);
      // Only accept join-link requests — skip member-added (non_admin_add) ones
      const linkOnly = pending.filter((r) =>
        r.method === "invite_link" || r.method === "unknown"
      );
