...
async def main():
    ...
    # 1. misc        — /start /help /games
    # 2. owner       — /feeset /fee /history /endall /cancel /reset (group admin + owner)
    # 3. leaderboard — /leaderboard
    # 4. profile     — /profile
    # 5. tournament  — /tournament + FSM
    # 6. battle      — /battle + paid battle FSM (before match so FSM states fire first)
    # 7. match       — game callbacks + guess number handler + rematch/newgame
    # 8. challenge   — /challenge (no catch-all text handler; always last)
    ...
    try:
        logger.info("PvP Gaming Arena Bot starting...")
