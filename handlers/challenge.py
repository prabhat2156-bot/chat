...
async def _expire_challenge(challenge_id: str, bot):
    ...
    # Also clean up the game-selection entry so the challenger can /challenge again cleanly
    await db.challenge_selections.delete_one({"challenger_id": ch["challenger_id"]})
    try:
        await bot.edit_message_text(
            chat_id=ch["group_id"], message_id=ch["message_id"],
            text="⏰ <b>Challenge Expired</b>", parse_mode="HTML", reply_markup=None,
        )
    except Exception:
        pass
...
async def _expire_confirmation(confirm_id: str, bot):
    ...
    try:
        await bot.edit_message_text(
            chat_id=conf["group_id"], message_id=conf["message_id"],
            text="⏰ <b>Match confirmation expired.</b> Challenge again to play.",
...
async def cmd_challenge(message: Message):
    ...
    if active_match:
        await message.answer("❌ You are already in an active match in this group.")
        return

    active_challenge = await get_active_challenge_for_user_in_group(challenger.id, group_id)
    if active_challenge:
        await message.answer("❌ You already have a pending challenge in this group.")
        return
    ...
    if opponent_username is None and opponent_id is None:
        await message.answer(f"❌ Could not find the player. Tag them directly.")
        return
...
async def cb_select_game(callback: CallbackQuery):
    ...
    challenge_msg = await callback.message.bot.send_message(
        chat_id=group_id,
        text=(
            f"⚔️ <b>Challenge Request</b>\n\n"
            f"👤 <b>Challenger:</b> @{challenger_name}\n"
            f"👤 <b>Opponent:</b> @{opponent_username}\n\n"
            f"🎮 <b>Game:</b> {game_name}\n\n"
            f"<i>@{opponent_username}, do you accept?</i>"
        ),
        parse_mode="HTML",
    ...
    await callback.message.bot.edit_message_reply_markup(
        chat_id=group_id, message_id=challenge_msg.message_id,
...
async def cb_challenge_accept(callback: CallbackQuery):
    ...
    if ch["status"] != "pending":
        msgs = {"accepted": "❌ Already accepted.", "declined": "❌ Declined.", "expired": "❌ Expired."}
    ...
    if active_match2:
        await callback.answer("❌ Challenger is already in a match.", show_alert=True)
        return
    ...
    try:
        await callback.message.edit_text(
            f"✅ <b>@{opponent_name} accepted the challenge!</b>",
            parse_mode="HTML", reply_markup=None,
        )
    except Exception:
        pass

    # ── STEP 2: Both players must confirm before match starts ──
    ...
    await callback.answer("✅ Accepted! Please confirm to start.")
...
async def _create_match_confirmation(
    ...
    text = _confirm_text(game_name, player1_name, player2_name, False, False)
...
def _confirm_text(game_name, p1_name, p2_name, p1_ready, p2_ready):
    ...
    return (
        ...
        f"👤 @{p1_name}  —  {p1_status}\n"
        f"👤 @{p2_name}  —  {p2_status}\n\n"
        f"<i>Both players press ⚡ I'm Ready to start!</i>"
    )
...
async def cb_match_confirm(callback: CallbackQuery):
    ...
    if conf.get(field):
        await callback.answer("✅ You already confirmed!", show_alert=False)
        return

    await db.match_confirmations.update_one(
        {"confirm_id": confirm_id}, {"$set": {field: True}}
    )
    conf[field] = True
    ...
    if both_ready:
        ...
        await db.match_confirmations.update_one(
            {"confirm_id": confirm_id}, {"$set": {"status": "started"}}
        )
        try:
            await callback.message.edit_text(
                f"⚡ <b>Both players ready!</b>\n🎮 Starting <b>{game_name}</b>...",
                parse_mode="HTML", reply_markup=None,
            )
        except Exception:
            pass
        ...
        try:
            ...
            pass
        await callback.answer("✅ Confirmed! Waiting for the other player...")
...
async def cb_challenge_decline(callback: CallbackQuery):
    ...
    timeout_manager.cancel_timeout(f"challenge_{challenge_id}")
    await db.challenges.update_one({"challenge_id": challenge_id}, {"$set": {"status": "declined"}})
