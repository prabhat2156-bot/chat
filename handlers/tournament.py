...
async def cmd_tournament(message: Message, state: FSMContext):
    ...
    if not await _is_group_admin(message.bot, group_id, user.id):
        await message.answer("<tg-emoji emoji-id='4956612582816351459'>❌</tg-emoji> Only group admins can create tournaments.")
        return
    ...
    if active_t:
        await message.answer("<tg-emoji emoji-id='4956612582816351459'>❌</tg-emoji> There is already an active tournament in this group.")
        return
    ...
    await message.answer(
        "<tg-emoji emoji-id='6194737030165959506'>🏆</tg-emoji> <b>Tournament Setup</b>\n\nStep 1: Select games for the tournament.\n(You can select multiple games)",
        parse_mode="HTML",
...
async def cb_tournament_game_done(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"<tg-emoji emoji-id='6194737030165959506'>🏆</tg-emoji> <b>Tournament Setup</b>\n\n"
        f"<tg-emoji emoji-id='4958610528588008305'>✅</tg-emoji> Games: {selected_names}\n\n"
...
async def cb_tournament_size(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"<tg-emoji emoji-id='6194737030165959506'>🏆</tg-emoji> <b>Tournament Setup</b>\n\n"
        f"<tg-emoji emoji-id='4958610528588008305'>✅</tg-emoji> Games: {selected_names}\n"
        f"<tg-emoji emoji-id='4958610528588008305'>✅</tg-emoji> Size: {size} players\n\n"
...
async def msg_custom_size(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("<tg-emoji emoji-id='4956612582816351459'>❌</tg-emoji> Please enter a valid number.")
        return

    if size < 4:
        await message.answer("<tg-emoji emoji-id='4956612582816351459'>❌</tg-emoji> Minimum size is 4 players.")
        return
    if size % 2 != 0:
        await message.answer("<tg-emoji emoji-id='4956612582816351459'>❌</tg-emoji> Size must be an even number.")
        return
...
def _render_join_board(current: int, total: int, games: List[str], final_format: str) -> str:
    ...
    lines = [
        "<tg-emoji emoji-id='6194737030165959506'>🏆</tg-emoji> <b>Tournament</b>",
        "",
        f"<tg-emoji emoji-id='5361741454685256344'>🎮</tg-emoji> Games: {game_names}",
        f"<tg-emoji emoji-id='5377444018328183495'>🏁</tg-emoji> Final: {final_format}",
        "",
...
async def cb_tournament_join(callback: CallbackQuery):
    ...
    if len(players) >= tournament["size"]:
        ...
        try:
            await callback.message.edit_text(
                "<tg-emoji emoji-id='4956596167451346576'>🎉</tg-emoji> <b>Tournament Full!</b>\nStarting...",
                parse_mode="HTML",
                reply_markup=None,
            )
        except Exception:
        ...
        try:
            ...
            pass
        await callback.answer(f"<tg-emoji emoji-id='4958610528588008305'>✅</tg-emoji> Joined! ({len(players)}/{tournament['size']})")
...
def _render_tournament_board(brackets: list, players: list) -> str:
    ...
    lines = ["<tg-emoji emoji-id='6194737030165959506'>🏆</tg-emoji> <b>Tournament Bracket</b>", ""]
    ...
    for r_idx, (round_matches, r_name) in enumerate(zip(brackets, round_names)):
        ...
        for m in round_matches:
            ...
            if status == "bye":
                ...
                lines.append(f"  <tg-emoji emoji-id='4958610528588008305'>✅</tg-emoji> @{p1_name} vs @{p2_name} → @{w_name}")
            elif status == "active":
                p1_name = p1["username"] if p1 else "?"
                p2_name = p2["username"] if p2 else "?"
                lines.append(f"  <tg-emoji emoji-id='5039775669496579510'>⚔️</tg-emoji> @{p1_name} vs @{p2_name} (playing)")
            elif status == "pending":
                p1_name = p1["username"] if p1 else "?"
                p2_name = p2["username"] if p2 else "?"
...
async def _finish_tournament(tournament_id: str, bot: Bot):
    ...
    champion_text = (
        f"━━━━━━━━━━━━━━\n\n"
        f"<tg-emoji emoji-id='4958725487682650920'>👑</tg-emoji> <b>TOURNAMENT CHAMPION</b>\n\n"
        f"<tg-emoji emoji-id='6194737030165959506'>🏆</tg-emoji> @{champion['username']}\n\n"
        f"<tg-emoji emoji-id='5361741454685256344'>🎮</tg-emoji> Tournament Game Set:\n{games_display}\n\n"
        f"<tg-emoji emoji-id='5870772616305839506'>👥</tg-emoji> Participants: {num_players}\n\n"
        f"━━━━━━━━━━━━━━"
    )
