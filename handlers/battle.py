...
from aiogram.filters import BaseFilter as Filter, Command
...
async def cmd_battle(message: Message):
    ...
    # FIX: clean up any stale /challenge game-selection keyboard for this user.
    # If the user ran /challenge but never picked a game, that message (with sel_game:
    # buttons) stays in chat.  Without this cleanup, pressing those buttons after
    # /battle shows "Session expired. Use /challenge again." — the exact bug reported.
    stale_sel = await db.challenge_selections.find_one({"user_id": challenger.id})
    if stale_sel:
        try:
            await message.bot.edit_message_reply_markup(
                chat_id=stale_sel["group_id"],
                message_id=stale_sel["select_msg_id"],
                reply_markup=None,
            )
        except Exception:
            pass
        await db.challenge_selections.delete_one({"user_id": challenger.id})
...
class _HasUpiWaiting(Filter):
    """Only matches when the sender has a pending battle UPI payout in this group.

    Using a proper Filter (instead of an early-return inside the handler) is
    critical: if the handler were registered with a broad filter (F.text) and
    merely returned early, aiogram would still mark the message as handled,
    silently dropping it before other routers (e.g. the Guess-Number handler in
    match.router) could process it.
    """
    ...
    async def __call__(self, message: Message) -> bool:
        if not message.from_user or not message.chat:
            return False
        db = get_db()
        doc = await db.battle_upi_waiting.find_one({
...
@router.message(_HasUpiWaiting(), F.text & ~F.text.startswith("/"))
