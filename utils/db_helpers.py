...
async def record_match_result(
    ...
    game: str, winner_id: Optional[int],
    is_draw: bool = False,
):
    db = get_db()
...
async def get_active_match_for_user_in_group(user_id: int, group_id: int) -> Optional[dict]:
    """Per-group check — 1 user can play in multiple groups simultaneously."""
    db = get_db()
    return await db.matches.find_one({
        "$or": [{"player1_id": user_id}, {"player2_id": user_id}],
        "status": "active",
...
async def get_active_challenge_for_user_in_group(user_id: int, group_id: int) -> Optional[dict]:
    db = get_db()
    return await db.challenges.find_one({
        "$or": [{"challenger_id": user_id}, {"opponent_id": user_id}],
        "status": "pending",
