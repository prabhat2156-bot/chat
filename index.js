...
const connectTimeouts = new Map(); // ── FIX: timeout track karne ke liye

setCallbacks({
  // ── FIX: ab error bhi receive karta hai ──
  onPairingCode: async (index, code, err) => {
    const cb = pendingPairingCbs.get(index);
    if (cb) {
      pendingPairingCbs.delete(index);
      // timeout clear karo
      const t = connectTimeouts.get(index);
      if (t) { clearTimeout(t); connectTimeouts.delete(index); }
      await cb(code, err);
    }
  },
  onReady: async (index) => {
...
bot.on("text", async (ctx) => {
  ...
  if (session.state === "awaiting_phone1" || session.state === "awaiting_phone2") {
    ...
    await ctx.reply(
      `⏳ *WA Account ${index + 1}* ke liye pairing code generate ho raha hai...\n_(Render par pehli baar 60-90 seconds lag sakte hain)_`,
      { parse_mode: "Markdown" }
    );

    // ── FIX: pairing callback ab error bhi handle karta hai ──
    pendingPairingCbs.set(index, async (code, err) => {
      if (err || !code) {
        const errMsg = err?.message || "Unknown error";
        console.error(`[Connect] Pairing failed for account ${index + 1}:`, errMsg);
        await ctx.reply(
          `❌ *Pairing code nahi mila — Account ${index + 1}*\n\n` +
          `*Error:* \`${errMsg}\`\n\n` +
          `*Possible reasons:*\n` +
          `• Phone number galat hai (check karein: country code + number)\n` +
          `• WhatsApp linked devices limit hit ho gayi\n` +
          `• Render par chromium properly nahi chal raha\n\n` +
          `Dobara try karein ya number check karein.`,
          {
            parse_mode: "Markdown",
            ...Markup.inlineKeyboard([[Markup.button.callback("🔙 Main Menu", "back_menu")]]),
          }
    ...
    // ── FIX: 120 second overall timeout — agar kuch bhi nahi hua toh user ko batao ──
    const timeoutHandle = setTimeout(async () => {
      if (pendingPairingCbs.has(index)) {
        pendingPairingCbs.delete(index);
        pendingReadyCbs.delete(index);
        connectTimeouts.delete(index);
        console.error(`[Connect] Timeout for account ${index + 1}`);
        try {
          await ctx.reply(
            `⏰ *Timeout — Account ${index + 1}*\n\n` +
            `2 minute mein response nahi aaya. Yeh aksar tab hota hai jab:\n` +
            `• Render par chromium install nahi\n` +
            `• Server cold start mein time lag raha hai\n\n` +
            `*Render Fix:* Build Command mein add karein:\n` +
    ...
    connectTimeouts.set(index, timeoutHandle);
    ...
    try {
      ...
      pendingReadyCbs.delete(index);
      await ctx.reply(
        `❌ *Connection Error — Account ${index + 1}*\n\n\`${err.message}\`\n\nDobara try karein.`,
        {
          parse_mode: "Markdown",
          ...Markup.inlineKeyboard([[Markup.button.callback("🔙 Wapas", "back_menu")]]),
        }
