...
function groupSelectionKeyboard(waIndex, groups, selectedIds) {
  ...
  const allSelected = shown.length > 0 && shown.every((g) => selSet.has(g.id));
  const rows = [];
  ...
  shown.forEach((g, i) => {
    rows.push([Markup.button.callback(
      `${selSet.has(g.id) ? "✅" : "◻️"} ${g.name.slice(0, 28)}`,
      `tg${waIndex}_${i}`,
    )]);
  });
  const count = selectedIds.length;
  rows.push([Markup.button.callback(
    `🚀 Confirm — ${count} group${count !== 1 ? "s" : ""} selected`,
    `confirm_wa${waIndex}`,
  )]);
...
async function handleConnectMenu(ctx, index) {
  ...
  const accNum = index + 1;

  if (status === "connected") {
    await ctx.reply(
      `📱 *WhatsApp Account ${accNum}*\n✅ Connected: \`${phone}\`\n\nKya logout karna hai?`,
      {
        parse_mode: "Markdown",
        ...Markup.inlineKeyboard([
          [Markup.button.callback(`🔌 Logout Karein`, `logout_${index}`)],
    ...
    await ctx.reply(
      `⏳ *WA Account ${accNum} connect ho raha hai...*\n\nWait karein ya reset karein:`,
      {
        parse_mode: "Markdown",
        ...Markup.inlineKeyboard([
          [Markup.button.callback("🔄 Reset Karein", `reset_${index}`)],
          [Markup.button.callback("🔙 Main Menu", "back_menu")],
        ]),
      },
    );
    ...
    await ctx.reply(
      ...
      `_Pairing code 10-20 sec mein aayega_`,
      {
        parse_mode: "Markdown",
...
// Logout — auth bhi clear hota hai
bot.action(/^logout_(\d)$/, async (ctx) => {
  ...
  await ctx.editMessageText(
    `✅ *WA Account ${index + 1} logout ho gaya!*\n_Ab naya number connect kar sakte hain._`,
    { parse_mode: "Markdown" },
  );
  await sleep(700);
...
// Reset — "number is processing" fix ke liye
bot.action(/^reset_(\d)$/, async (ctx) => {
  await ctx.answerCbQuery("Reset ho raha hai...");
  const index = parseInt(ctx.match[1]);
  await disconnectAccount(index);
  await ctx.editMessageText(
    `✅ *WA Account ${index + 1} reset ho gaya!*\n\nAb dobara number bhejein.`,
    { parse_mode: "Markdown" },
  );
  await sleep(500);
  const userId = ctx.from.id;
...
bot.action("menu_delay", async (ctx) => {
  ...
  await ctx.reply(
    `⏱️ *Message Delay Set Karein*\nCurrent: *${session.delaySeconds} seconds*\n\nHar group ke baad kitna wait karein?`,
    {
      ...
      ...Markup.inlineKeyboard([
        [
          Markup.button.callback("1s", "delay_1"),
          Markup.button.callback("3s", "delay_3"),
          Markup.button.callback("5s", "delay_5"),
          Markup.button.callback("10s", "delay_10"),
          Markup.button.callback("30s", "delay_30"),
        ],
        [Markup.button.callback("🔙 Main Menu", "back_menu")],
      ]),
    },
...
bot.action("menu_status", async (ctx) => {
  ...
  await ctx.reply(
    `📊 *Bot Status*\n\n` +
    `*📱 WA 1:* ${statusEmoji(getStatus(0))} ${getStatus(0).toUpperCase()}` +
    (getPhone(0) ? ` — \`${getPhone(0)}\`` : "") + "\n" +
    `*📱 WA 2:* ${statusEmoji(getStatus(1))} ${getStatus(1).toUpperCase()}` +
    (getPhone(1) ? ` — \`${getPhone(1)}\`` : "") + "\n\n" +
    `*Script 1:* ${sc1.length} messages\n` +
    `*Script 2:* ${sc2.length} messages\n\n` +
    `*⏱️ Delay:* ${session.delaySeconds}s\n` +
    `*⏰ Duration:* ${formatDuration(session.scheduleDays)}\n` +
    `*🔄 Repeat:* Har ${formatRepeat(session.repeatHours)}\n` +
...
bot.action("confirm_wa1", async (ctx) => {
  ...
  if (s2 === "connected" && sc2.length > 0) {
    ...
    updateSession(userId, {
      state: "selecting_wa2",
      wa2Groups: groups,
      selectedWa2Ids: groups.map((g) => g.id),
...
async function launchBroadcastLoop(ctx, userId) {
  ...
  const scheduleText = scheduleDays === null
    ? `♾️ Nonstop — Har *${formatRepeat(repeatHours)}* mein repeat`
  ...
  const runLoop = async () => {
    while (!flag.stopped) {
      ...
      const buildStatus = (extra = "⏳ Chal raha hai...") =>
        ...
        `📱 WA1: \`${getPhone(0) || "—"}\`\n📱 WA2: \`${getPhone(1) || "—"}\`\n\n` +
        `📍 *Groups:* ${total}\n📤 *Sent:* ${sent} / ${total}\n❌ *Failed:* ${failed}\n` +
        `🔁 *Cycles:* ${getSession(userId).broadcastCycles}\n⏳ *Time Left:* ${formatTimeLeft(endTime)}\n\n${extra}`;
...
bot.on("text", async (ctx) => {
  ...
  if (session.state === "awaiting_phone1" || session.state === "awaiting_phone2") {
    ...
    await ctx.reply(
      `⏳ *WA Account ${index + 1}* ke liye pairing code generate ho raha hai...\n_(10-20 seconds mein code aayega)_`,
      { parse_mode: "Markdown" },
    );
...
app.get("/", (_req, res) => {
  res.send(`
    <html><body style="font-family:sans-serif;text-align:center;padding:50px;background:#0a0a0a;color:#fff">
      <h2>✅ WhatsApp Broadcast Bot v3.1</h2>
      <p style="color:#4ade80">Bot chal raha hai 🟢 (Baileys)</p>
...
app.listen(PORT, () => console.log(`🌐 Web server ready — port ${PORT}`));

// ─── Self-ping ─────────────────────────────────────────────────────────
function selfPing() {
  const selfUrl = process.env.RENDER_EXTERNAL_URL || process.env.SELF_URL;
  if (!selfUrl) return;
  const fullUrl = selfUrl.startsWith("http") ? selfUrl : `https://${selfUrl}`;
  const client = fullUrl.startsWith("https") ? https : http;
  client.get(`${fullUrl}/health`, (res) => {
    console.log(`[Ping] ✅ ${res.statusCode} — ${new Date().toLocaleTimeString()}`);
  }).on("error", (err) => console.error("[Ping] ❌", err.message));
}
setTimeout(() => { selfPing(); setInterval(selfPing, 2 * 60 * 1000); }, 60 * 1000);

// ─── Bot launch ────────────────────────────────────────────────────────
bot.launch({ dropPendingUpdates: true }).then(() => {
  console.log("✅ WhatsApp Broadcast Bot v3.1 (Baileys) chal raha hai!");
});
