...
const {
  setCallbacks, toJid, numberMatches, getStatus, getPhone, getAllStatuses, getConnectedCount,
...
bot.action(/^gm_pending_(\d+)$/, async (ctx) => {
  ...
  const { list: pending, error: pendingErr } = await getGroupPendingRequests(accIdx, groupId);
  try { await ctx.telegram.deleteMessage(ctx.chat.id, loadMsg.message_id); } catch {}

  const grpIdx = s.gmGroups?.findIndex((g) => g.id === groupId) ?? 0;

  if (!pending.length) {
    const errNote = pendingErr ? `\n\n⚠️ _API Error: ${pendingErr}_` : "";
    await ctx.reply(
      `⏳ *Pending Requests — ${groupName}*\n\n✅ Koi pending request nahi hai!${errNote}\n\n_Note: Pending requests tabhi aate hain jab group mein "Approval required" setting ON ho._`,
      { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🔙 Group Menu", `gm_grp_${accIdx}_${grpIdx}`)]]) },
    );
    return;
  }

  const linkReqs = pending.filter((p) => p.method === "invite_link" || p.method === "unknown");
  const addedReqs = pending.filter((p) => p.method === "non_admin_add");

  let text = `⏳ *Pending Requests — ${groupName}*\nTotal: ${pending.length}\n\n`;

  if (linkReqs.length) {
    text += `*🔗 Join Link Requests (${linkReqs.length}):*\n`;
    text += linkReqs.map((p) => `📩 \`${p.number}\``).join("\n");
    text += "\n\n";
  }
  if (addedReqs.length) {
    text += `*👤 Member-Added (${addedReqs.length}):*\n`;
    text += `_(Auto-Accept inhe skip karta hai)_\n`;
    text += addedReqs.map((p) => `➕ \`${p.number}\``).join("\n");
  }
...
bot.on("text", async (ctx) => {
  ...
  if (s.awaitingNumberForAction) {
    ...
    if (action === "promote") {
      // Search members using flexible suffix match (handles missing country code)
      const members = await getGroupMembers(accIdx, groupId);
      const found = members.find((m) => numberMatches(m.number, phone));

      if (found) {
        if (found.admin) {
          try { await ctx.telegram.deleteMessage(ctx.chat.id, waitMsg.message_id); } catch {}
          await ctx.reply(
            `ℹ️ \`${found.number}\` *pehle se admin hai!*`,
            { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🔙 Group Menu", `gm_grp_${accIdx}_${grpIdx}`)]]) },
          );
          return;
        }
        const result = await promoteParticipant(accIdx, groupId, found.id);
        try { await ctx.telegram.deleteMessage(ctx.chat.id, waitMsg.message_id); } catch {}
        if (result.ok) {
          await ctx.reply(
            `✅ *\`${found.number}\` ko admin bana diya!*\n👑 Group: ${groupName}`,
            { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🔙 Group Menu", `gm_grp_${accIdx}_${grpIdx}`)]]) },
          );
        } else {
        ...
        const { list: pending } = await getGroupPendingRequests(accIdx, groupId);
        const pendingFound = pending.find((p) => numberMatches(p.number, phone));
        ...
        if (pendingFound) {
          ...
          if (promoteResult.ok) {
            await ctx.reply(
              `✅ *\`${pendingFound.number}\` ko accept karke admin bana diya!*\n📩 Pending request accept ki\n👑 Admin promote kiya\n🏠 Group: ${groupName}`,
              { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🔙 Group Menu", `gm_grp_${accIdx}_${grpIdx}`)]]) },
            );
          } else {
            await ctx.reply(
          ...
          try { await ctx.telegram.deleteMessage(ctx.chat.id, waitMsg.message_id); } catch {}
          // Show first few stored numbers for debugging
          const sample = members.slice(0, 5).map((m) => `\`${m.number}\``).join(", ");
          await ctx.reply(
            `❌ *\`${phone}\` nahi mila!*\n\nNa members mein hai, na pending requests mein.\n\n` +
            `_Aapne jo number enter kiya: \`${phone}\`_\n` +
            `_Group mein stored numbers example: ${sample || "koi member nahi"}_\n\n` +
            `Country code sahi hai? Jaise India ke liye \`91XXXXXXXXXX\` format use karein.`,
            { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🔙 Group Menu", `gm_grp_${accIdx}_${grpIdx}`)]]) },
          );
        }
      }
      ...
      if (!found.admin) {
        await ctx.reply(
          `ℹ️ *\`${found.number}\` admin nahi hai!*\nSirf admins ko demote kiya ja sakta hai.`,
          { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🔙 Group Menu", `gm_grp_${accIdx}_${grpIdx}`)]]) },
        );
        return;
      }

      const result = await demoteParticipant(accIdx, groupId, found.id);
      if (result.ok) {
        await ctx.reply(
          `✅ *\`${found.number}\` ko admin se hata diya!*\n🏠 Group: ${groupName}`,
          { parse_mode: "Markdown", ...Markup.inlineKeyboard([[Markup.button.callback("🔙 Group Menu", `gm_grp_${accIdx}_${grpIdx}`)]]) },
        );
      } else {
