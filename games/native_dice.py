...
def render_board(match: dict) -> str:
    ...
    p1_status = f"✅ <b>{state['player1_value']}</b>" if state["player1_rolled"] else "⏳ Not Rolled"
    p2_status = f"✅ <b>{state['player2_value']}</b>" if state["player2_rolled"] else "⏳ Not Rolled"
...
def render_result(match: dict) -> str:
    ...
    lines = [
        ...
        "",
        f"👤 @{p1_name}  →  <b>{p1_val}</b>",
        f"👤 @{p2_name}  →  <b>{p2_val}</b>",
        "",
    ]
    if is_draw:
