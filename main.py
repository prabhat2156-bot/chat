...
# ---------------------------------------------------------------------------
# Fast VCF parser — does NOT use vobject (which is very slow on large files).
# Uses a plain-text state machine: handles any standard VCF v2.1/3.0/4.0.
# ---------------------------------------------------------------------------
def _parse_vcf_fast(text: str) -> List[Tuple[str, str]]:
    ...
    for line in text.splitlines():
        ...
        if ul == "BEGIN:VCARD":
            ...
            if in_vcard and current_phone:
                contacts.append((current_name or "Contact", current_phone))
            in_vcard = False
        elif in_vcard:
            if ul.startswith("FN:"):
                current_name = line[3:].strip()
            elif ul.startswith("FN;"):
                # FN;CHARSET=...:value
                colon = line.find(":")
                if colon != -1:
                    current_name = line[colon + 1:].strip()
            elif ul.startswith("TEL") and not current_phone:
                colon = line.find(":")
                if colon != -1:
                    phone = clean_phone(line[colon + 1:])
                    if phone:
                        current_phone = phone
    return contacts
...
def parse_xlsx_contacts(data: bytes) -> List[Tuple[str, str]]:
    """Memory-efficient XLSX parser using read_only mode."""
    contacts = []
    try:
...
# ---------------------------------------------------------------------------
# Output generators — all work on in-memory buffers but built efficiently
# ---------------------------------------------------------------------------
...
def contacts_to_vcf(contacts: List[Tuple[str, str]]) -> bytes:
    ...
    for name, phone in contacts:
        ...
        buf.write(f"FN:{name}\n")
        buf.write(f"TEL;TYPE=CELL:{phone}\n")
        buf.write("END:VCARD\n\n")
...
def analyze_contacts(contacts: List[Tuple[str, str]]) -> dict:
    """
    Analyze contacts efficiently.
    Country detection is limited to the first 10 000 unique numbers
    to avoid hanging on files with 100 000+ entries.
    """
    seen_digits: set = set()
    clean_count = 0
    duplicate_count = 0
    ...
    for name, phone in contacts:
        ...
        if digits in seen_digits:
            ...
            clean_count += 1
            if clean_count <= COUNTRY_SAMPLE_LIMIT:
    ...
    return {
        ...
        "country_sampled": min(clean_count, COUNTRY_SAMPLE_LIMIT),
    }
...
async def fa_upload(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await status_msg.edit_text("⏳ Analyzing contacts (this may take a moment for large files)...")

        # Run CPU-heavy work off the event loop so bot stays responsive
        contacts, analysis = await asyncio.to_thread(
            lambda: (
                (c := parse_file(data, doc.file_name or "file.vcf")),
                analyze_contacts(c),
            )[-1]  # returns tuple (contacts, analysis)
        )
        # Re-run properly (lambda trick is fragile) — use a helper function
        def _do_analysis():
            c = parse_file(data, doc.file_name or "file.vcf")
            a = analyze_contacts(c)
            return c, a

        contacts, analysis = await asyncio.to_thread(_do_analysis)
        ...
        sampled_note = ""
        if analysis["clean"] > analysis["country_sampled"]:
            sampled_note = f"\n_⚠️ Country data from first {analysis['country_sampled']:,} unique numbers (large file)_"
...
async def fc_upload(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        contacts = await asyncio.to_thread(parse_file, data, doc.file_name or "file.vcf")
...
async def fc_format(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    status = await update.message.reply_text(f"⏳ Converting {len(contacts):,} contacts to {fmt.upper()}...")
    try:
        out_data = await asyncio.to_thread(contacts_to_format, contacts, fmt)
...
async def qv_phone(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(msg, reply_markup=more_finish_kb(), parse_mode="Markdown")
    return QV_MORE
...
async def qv_finish(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    vcf_data = await asyncio.to_thread(contacts_to_vcf, contacts)
...
async def vm_upload(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        contacts = await asyncio.to_thread(parse_file, data, doc.file_name or "file.txt")
...
async def vm_show_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    group_display = f"{group_name} (starts from {group_start})" if group_name else "(none)"
...
async def vm_generate(update, context):
    """Generate VCF files. Builds each chunk in a thread, sends files one by one."""
    vm = context.user_data["vm"]
    phones = vm["phones"]
    basename = vm["basename"]
    ...
    status = await update.message.reply_text(
        f"⏳ Generating {total_files:,} VCF file(s) for {total:,} contacts..."
    )
    ...
    for chunk_start in range(0, total, per_file):
        chunk_phones = phones[chunk_start:chunk_start + per_file]
        current_file_num = file_start + file_idx
        current_group_num = group_start + file_idx

        def _build_chunk(chunk_phones=chunk_phones, contact_num=contact_num,
                         current_group_num=current_group_num):
            contacts = []
            num = contact_num
            for phone in chunk_phones:
                if group_name:
                    name = f"{contact_name} {num} {group_name} {current_group_num}"
                else:
                    name = f"{contact_name} {num}"
                contacts.append((name, phone))
                num += 1
            return contacts_to_vcf(contacts)

        vcf_data = await asyncio.to_thread(_build_chunk)
        out_name = f"{basename}{current_file_num}.vcf"

        await update.message.reply_document(
            document=InputFile(io.BytesIO(vcf_data), filename=out_name),
            caption=f"📦 {out_name}: {len(chunk_phones):,} contacts",
        )

        contact_num += len(chunk_phones)
        file_idx += 1
        sent += 1

        # Update progress every 10 files to avoid Telegram rate limits
        if sent % 10 == 0 and sent < total_files:
            try:
                await status.edit_text(
                    f"⏳ Progress: {sent}/{total_files} files sent..."
                )
            except Exception:
                pass

    await status.edit_text(
        f"✅ Done! Generated {file_idx:,} file(s) with {total:,} contacts total."
    )
...
async def sf_count(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    total = len(contacts)
    total_parts = (total + count - 1) // count

    status = await update.message.reply_text(
        f"⏳ Splitting {total:,} contacts into {total_parts:,} file(s)..."
    )
    file_num = 1
    sent = 0
    ...
    for chunk_start in range(0, total, count):
        ...
        out_data = await asyncio.to_thread(contacts_to_format, chunk, ext)
        ...
        file_num += 1
        sent += 1
        if sent % 10 == 0 and sent < total_parts:
            try:
                await status.edit_text(f"⏳ Progress: {sent}/{total_parts} files sent...")
            except Exception:
                pass
...
async def mf_upload(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        file = await context.bot.get_file(doc.file_id)
        data = bytes(await file.download_as_bytearray())
        new_contacts = await asyncio.to_thread(parse_file, data, doc.file_name or "file.vcf")
...
async def fe_save(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    out_data = await asyncio.to_thread(contacts_to_format, contacts, ext)
...
async def rc_upload(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        file = await context.bot.get_file(doc.file_id)
        data = bytes(await file.download_as_bytearray())
        contacts = await asyncio.to_thread(parse_vcf_contacts, data)
        if not contacts:
            await status.edit_text("❌ No contacts found.")
            return RC_UPLOAD
...
async def rc_all_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    contacts = context.user_data.get("rc_contacts", [])
    status = await update.message.reply_text(f"⏳ Renaming {len(contacts):,} contacts...")

    def _do_rename():
        renamed = [(f"{base_name} {i}", phone) for i, (_, phone) in enumerate(contacts, 1)]
        return renamed, contacts_to_vcf(renamed)

    renamed, vcf_data = await asyncio.to_thread(_do_rename)
...
async def st_set_group_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if text == "⏭ Skip":
        settings["group_name"] = ""
        await update.message.reply_text("✅ Group name cleared.", reply_markup=settings_kb(settings))
        return ST_MAIN
...
def main():
    if BOT_TOKEN == "YOUR_BOT_TOKEN_HERE":
        raise RuntimeError(
            "Set your bot token: export BOT_TOKEN=your_token  (or edit BOT_TOKEN in bot.py)"
        )
